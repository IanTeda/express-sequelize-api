...
describe('Unit :: Database :: Model :: ResetToken', () => {
  // Test instances
  ...
  describe('00. ResetToken Test Checks:', () => {
    it('expect resetToken.factory to generate a reset token with ID', async () => {
    ...
    it('expect truncate to clean ResetToken table with each test', async () => {
  ...
  describe('02. ResetToken Validation Checks:', () => {
    it('expect ResetToken.create to throw an error on null userId', (done) => {
      ...
      ResetToken.create(fakeResetTokenData)
        .catch((error) => {
          expect(error).to.have.property('message').to.contain('DATABASE ERROR: Reset Token UserId cannot be null.');
        })
        .then(done, done);
    });
    ...
    it('expect ResetToken.create to throw an error on empty userId', (done) => {
      ...
      ResetToken.create(fakeResetTokenData)
        .catch((error) => {
          expect(error).to.have.property('message').to.contain('DATABASE ERROR: Reset Token UserId cannot be empty.');
        })
        .then(done, done);
    });
